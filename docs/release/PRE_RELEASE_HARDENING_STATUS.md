...
| EDGE-01 | P1 | `create-video-session` early verification gate deployment | OPEN | GitHub source requires `providers.verified=true` before creating a Meet space (`e279b108`). Live Edge Function remains on the older deployed version because direct connected deployment was blocked. DB trigger protection is live, so unverified hosts cannot persist a session. Deploy the patched function before release to avoid an orphan Meet space being created before DB rejection. |
