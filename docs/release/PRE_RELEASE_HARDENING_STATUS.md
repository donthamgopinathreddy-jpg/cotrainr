...
| HEALTH-01 | P0 | Android health-source correctness | CLOSED | Health Connect/Apple Health remains the only movement source; no sensor fallback was added. Android initialization now fails closed when Health Connect is unavailable or no movement permission is usable (`90ffbc39`). Metrics sync no longer persists empty snapshots when the source is unavailable and preserves existing positive daily totals when an individual Health Connect read is denied/transiently empty (`11f9b855`). Home already renders cached real data or an unavailable dash rather than a fake measured zero. Regression coverage added in `test/utils/health_metric_display_test.dart` (`f0db81fe`). Local/CI test execution remains part of final build verification. |
...
| RPC-01 | P1 | Remaining anonymous SECURITY DEFINER functions | VERIFIED | Three ACL hardening waves reduced anonymous SECURITY DEFINER exposure from 29 to 6. The remaining six are three intentional pre-auth helpers (`check_user_id_availability`, `is_username_available`, `rpc_resolve_login_identifier`) and three PostGIS-owned `st_estimatedextent` overloads. App-only discover/profile/video/messaging RPCs no longer require anonymous execution. PostGIS-owned functions are not modified blindly. |
...
- Anonymous SECURITY DEFINER functions: 6 remaining after ACL hardening (down from 29): 3 intentional pre-auth helpers + 3 PostGIS-owned functions.
