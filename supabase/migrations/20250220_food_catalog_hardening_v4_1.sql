-- =============================================================================
-- FOOD CATALOG HARDENING V4.1
-- Additive/idempotent patches. Run after the base food catalog migration
-- (20250215_add_food_catalog.sql) and any earlier hardening.
-- Fixes:
-- 1) DROP/CREATE INDEX as plain SQL (idempotent); DO blocks only for ALTER TABLE
-- 2) Guard admin_search_foods ALTER/REVOKE/GRANT with to_regprocedure
-- 3) Dedupe: verify normalized_name exists before use; raise clear exception if missing
-- 4) Dedupe: compute loser->winner mapping once (temp table), reuse for remaps and deletes
-- =============================================================================
...
-- 1) Verify normalized_name exists; raise if missing
-- 2) Compute loser->winner mapping once (temp table)
-- 3) Remap food_portions.food_id, meal_items.food_id
-- 4) Delete loser food rows
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'foods' AND column_name = 'normalized_name'
  ) INTO v_has_normalized_name;

  IF NOT v_has_normalized_name THEN
    RAISE EXCEPTION 'foods.normalized_name is missing; run migration step A or fix drift first.';
  END IF;
  ...
  v_has_meal_items_food_id := (
    to_regclass('public.meal_items') IS NOT NULL
    AND EXISTS (
      SELECT 1 FROM information_schema.columns
  ...
  IF v_has_created_at THEN
    -- Compute loser->winner mapping once; remap and delete
    CREATE TEMP TABLE IF NOT EXISTS _food_dedupe_loser_winner (loser_id uuid, winner_id uuid) ON COMMIT DROP;
    TRUNCATE TABLE _food_dedupe_loser_winner;
    -- Schema sanity check: verify loser_id, winner_id (uuid)
    IF NOT (
      EXISTS (SELECT 1 FROM pg_attribute a JOIN pg_class c ON a.attrelid = c.oid
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname LIKE 'pg_temp%' AND c.relname = '_food_dedupe_loser_winner'
          AND a.attname = 'loser_id' AND a.atttypid = 'uuid'::regtype AND NOT a.attisdropped AND a.attnum > 0)
      AND EXISTS (SELECT 1 FROM pg_attribute a JOIN pg_class c ON a.attrelid = c.oid
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname LIKE 'pg_temp%' AND c.relname = '_food_dedupe_loser_winner'
          AND a.attname = 'winner_id' AND a.atttypid = 'uuid'::regtype AND NOT a.attisdropped AND a.attnum > 0)
    ) THEN
      RAISE EXCEPTION '_food_dedupe_loser_winner schema mismatch: expected loser_id uuid, winner_id uuid. Close editor session and rerun migration.';
    END IF;
    ...
    INSERT INTO _food_dedupe_loser_winner (loser_id, winner_id)
    SELECT r_loser.id, r_winner.id
    FROM ranked r_loser
    ...
    IF to_regclass('public.food_portions') IS NOT NULL THEN
      ...
      FROM _food_dedupe_loser_winner lw
    ...
    WHERE id IN (SELECT loser_id FROM _food_dedupe_loser_winner);
...
-- =============================================================================
-- C) Drop old index, create unique index (guarded; DDL via EXECUTE)
-- =============================================================================
DO $$
BEGIN
  IF to_regclass('public.foods') IS NOT NULL THEN
    EXECUTE 'DROP INDEX IF EXISTS public.uq_foods_lower_name';
    EXECUTE 'DROP INDEX IF EXISTS public.idx_foods_lower_name';
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS uq_foods_normalized_name ON public.foods (normalized_name)';
  END IF;
END
$$;

-- =============================================================================
-- D) Keep portions partial unique index (guarded; DDL via EXECUTE)
-- =============================================================================
DO $$
BEGIN
  IF to_regclass('public.food_portions') IS NOT NULL THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS uq_food_portions_one_default_per_food ON public.food_portions (food_id) WHERE is_default = true';
  END IF;
END
$$;
...
-- =============================================================================
-- H) admin_search_foods: security hardening (guarded with to_regprocedure)
-- =============================================================================
DO $$
BEGIN
  IF to_regprocedure('public.admin_search_foods(text, integer, boolean)') IS NOT NULL THEN
    EXECUTE 'ALTER FUNCTION public.admin_search_foods(text, integer, boolean) OWNER TO postgres';
    EXECUTE 'REVOKE ALL ON FUNCTION public.admin_search_foods(text, integer, boolean) FROM PUBLIC';
    EXECUTE 'REVOKE ALL ON FUNCTION public.admin_search_foods(text, integer, boolean) FROM authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.admin_search_foods(text, integer, boolean) TO service_role';
  END IF;
END
$$;
